...
async def delete_all_user_preferences(user_id:int) -> None:
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                await cur.execute("Delete FROM userNewsSourcePreferences where userID = %s", (user_id,))
                await conn.commit()
    ...
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                insert_stmt = "INSERT INTO userNewsSourcePreferences (userID, CorporationID, Preference) VALUES (%s, %s, TRUE)"
...
async def add_user_preference(user_id: int, corporation_id: int) -> None:
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                insert_stmt = "INSERT INTO userNewsSourcePreferences (userID, CorporationID, Preference) VALUES (%s, %s, TRUE) ON DUPLICATE KEY UPDATE Preference = TRUE"
...
async def remove_user_preference(user_id: int, corporation_id: int) -> None:
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                delete_stmt = "DELETE FROM userNewsSourcePreferences WHERE userID = %s AND CorporationID = %s"
...
async def add_user_blacklist(user_id: int, corporation_id: int) -> None:
    ...
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                insert_stmt = "INSERT INTO userNewsSourcePreferences (userID, CorporationID, Preference) VALUES (%s, %s, FALSE) ON DUPLICATE KEY UPDATE Preference = 0"
                await cur.execute(insert_stmt, (user_id, corporation_id))
                await conn.commit()
...
async def get_user_preference(user_id: int, corporation_id: int) -> bool:
    async with aiomysql.create_pool(**conn_params) as pool:
        async with pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                query_stmt = "SELECT * FROM userNewsSourcePreferences WHERE userID = %s AND CorporationID = %s"
